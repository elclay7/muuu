(() => {
  ...
  function confirmDeleteRecord(group, index) {
    const records = schedules[group].records;
    if (records.length <= 1) return;
    const label = group === "tomas" ? "toma" : "extracción";
    openConfirmDialog({
      title: `¿Eliminar ${label}?`,
      message: "Este registro se borrará permanentemente. No se puede deshacer.",
      onConfirm: () => {
  ...
  function renderGroup(group) {
    ...
    records.forEach((record, index) => {
      ...
      deleteButton.addEventListener("click", () => confirmDeleteRecord(group, index));
      if (records.length <= 1) {
  ...
  function init() {
    ...
    document.getElementById("btn-reset").addEventListener("click", () => {
      openConfirmDialog({
        ...
        message: "Se eliminarán todas las tomas y extracciones de este día. No se puede deshacer.",
        onConfirm: async () => {
    ...
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape" && !document.getElementById("confirm-dialog").hidden) {
        closeConfirmDialog();
      }
    });
